...
class TigerSimulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=True, log_metrics=False, optimized=False,vanilla = False):
        ...
        self.blocksize = 100
        self.size = size if size is not None else (3 * self.blocksize,int(2.2*self.blocksize))
            ...
            if vanilla:
                if a.learning:
                    if self.optimized: # Whether the user is optimizing the parameters and decay functions
                        self.log_filename = os.path.join("logs", "qtiger_sim_improved-learning.csv")
                        self.table_filename = os.path.join("logs","qtiger_sim_improved-learning.txt")
                    else: 
                ...
                if a.learning:
                    if self.optimized: # Whether the user is optimizing the parameters and decay functions
                        self.log_filename = os.path.join("logs", "tiger_sim_improved-learning.csv")
                        self.table_filename = os.path.join("logs","tiger_sim_improved-learning.txt")
                    else: 
    ...
    def render(self, trial, testing=False):
        ...
        self.pygame.draw.rect(self.screen,self.colors['black'],self.pygame.Rect(self.blocksize,self.blocksize,self.blocksize,self.blocksize))
        self.pygame.draw.rect(self.screen,self.colors['white'],self.pygame.Rect(self.blocksize+2,self.blocksize + 2,self.blocksize - 4, self.blocksize -4))
        
        tigerx = 0
        if self.env.tigerlocation == 'right':
            tigerx = self.blocksize * 2
        self.pygame.draw.rect(self.screen,self.colors['red'],self.pygame.Rect(tigerx,0,self.blocksize,self.blocksize/2))
        ...
        for agent, state in self.env.agent_states.items():
            ...
            if location in [0,1,2,6,7,8]:
                agent_pos = (self.blocksize + (self.blocksize /3),self.blocksize + self.blocksize/2)
                if location in [1,7]:
                    self.pygame.draw.rect(self.screen,self.colors['magenta'],self.pygame.Rect(2*self.blocksize,self.blocksize,self.blocksize/2,self.blocksize))
                else:
                    if location in [2,8]:
                        self.pygame.draw.rect(self.screen,self.colors['cyan'],self.pygame.Rect((2*self.blocksize) + self.blocksize/2,self.blocksize,self.blocksize/2,self.blocksize))
                        
            else:
                if location in [3,9]:
                    agent_pos = (int(1.5*self.blocksize),self.blocksize/2)
                else:
                    if location == 4:
                        if self.env.tigerlocation == 'left':
                            agent_pos = (2*self.blocksize + (self.blocksize/2), self.blocksize/2)
                        else:
                            agent_pos = (self.blocksize/2,self.blocksize/2)
                    else:
                        if self.env.tigerlocation == 'left':
                            agent_pos = ((self.blocksize/2), self.blocksize/2)
                        else:
                            agent_pos = ((2 * self.blocksize) + self.blocksize/3,self.blocksize/3)
        ...
        self.renderMindState(self.blocksize/2,1.5*self.blocksize)
