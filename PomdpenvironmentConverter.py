...
class POMDPEnvironmentConverter():
    ...
    def getState(self):
        tollerance = 0.0000001 #floating point errors.
        MAGNIFICATION = 100000 #magnify values for debuging purposes.
        for state in self.pomdpStates:
            ...
            if sumtriangles <= triangle.getArea():
                return state
            if sumtriangles < triangle.getArea():
                for triangle in triangles:
                    print "This should be impossible value is {} less than target {}".format(triangle.getArea() - sumtriangles,triangle.getArea())
                    print "printing triangle coords {}".format(triangle.coords)
    ...
    def translateNDpointto2D(self,point):
        ...
        xcoord = ycoord = 0.0
        direction = []
        for axis in range(0,states):
            direction.append(baserad*axis)    
        for indx in range(0,states):
            xcoord = xcoord + (point[indx] * math.cos(direction[indx]*math.pi))
            ycoord = ycoord + (point[indx] * math.sin(direction[indx]*math.pi))
