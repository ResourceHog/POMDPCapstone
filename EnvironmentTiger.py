...
class Environment4x1(object):
    def __init__(self):
        self.valid_actions = ['left', 'right','silence','tiger']
        self.valid_observables = ['blue', 'green']
        self.states = OrderedDict()
        self.tigerlocation = Random.randint(0,1)
        self.valid_inputs = {'state': self.states.keys()}
        self.agent_states = OrderedDict()
        self.states[0] = uniqueState(self.valid_observables[2]) #possible tiger state
        self.states[1] = uniqueState(self.valid_observables[2]) #possible tiger state
        self.states[2] = uniqueState(self.valid_observables[2]) #hallway
        self.states[3] = uniqueState(self.valid_observables[2]) #listening post
        self.states[self.tigerlocation].statetype = self.valid_observables[3]
        self.done = False
    ...
    def step(self):
        ...
        self.timelapsed += 1
        if self.tigerlocation == 1:
            safety = 0
        else:
            safety = 1
        if self.agent_states[self.primary_agent]['location'] == safety:
            self.success = True
            self.trial_data['success'] = self.success
            self.done = True
        else:
            if self.agent_states[self.primary_agent]['location'] == self.tigerlocation || self.timelapsed >= self.TIMELIMIT:
                self.done = True
    ...
    def administerReward(self):
        safety = 0 ? self.tigerlocation == 1 : 1
        if self.agent_states[self.primary_agent]['location'] == safety :
